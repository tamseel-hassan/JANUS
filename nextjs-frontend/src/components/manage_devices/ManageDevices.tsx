...
import { Network, Server, Shield, Wifi, HardDrive, Cpu, Plus, Edit, Trash2, Link as LinkIcon, Box, ShieldCheck, ShieldAlert, Search, MapPin } from "lucide-react";
...
export function ManageDevices() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    const data = await safeFetch<{ devices: Device[]; links: Link[]; models: Record<string, string[]> }>(
    ...
    <div className="space-y-6 max-w-7xl mx-auto relative pb-10">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        ...
        <div className="flex space-x-3">
          <Button onClick={() => { setEditingDevice(null); setDeviceForm(emptyDevice); setCustomModel(''); setShowDeviceModal(true); }}>
            <Plus className="w-5 h-5 mr-2" /> Add Device
          </Button>
          <Button variant="ghost" className="bg-blue-600 hover:bg-blue-500 text-foreground px-4 py-2 font-bold flex items-center" onClick={() => { setEditingLink(null); setLinkForm(emptyLink); setShowLinkModal(true); }}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <StatCard title="Device Types" value={Object.keys(typeCounts).length} color="text-accent-primary" />
        <StatCard title="Network Links" value={links.length} color="text-blue-400" />
      </div>
                ...
                {filteredDevices.map(d => (
                  <tr key={d.id} className="hover:bg-bg-card/50 transition-colors">
                    ...
                    <td className="px-6 py-3 text-right">
                      <Button variant="outline-primary" size="icon" onClick={() => { setEditingDevice(d); setDeviceForm(d); setCustomModel(''); setShowDeviceModal(true); }} className="mr-2" title="Edit">
                        <Edit className="w-4 h-4" />
                      </Button>
                      <Button variant="outline-danger" size="icon" onClick={() => handleDeleteDevice(d.id, d.name)} title="Delete">
                ...
                {filteredLinks.map(l => (
                  <tr key={l.id} className="hover:bg-bg-card/50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-right">
                      <Button variant="outline-primary" size="icon" onClick={() => { setEditingLink(l); setLinkForm(l); setShowLinkModal(true); }} className="mr-2" title="Edit">
                        <Edit className="w-4 h-4" />
                      </Button>
                      <Button variant="outline-danger" size="icon" onClick={() => handleDeleteLink(l.id, l.name)} title="Delete">
                        <Trash2 className="w-4 h-4" />
                      </Button>
                    </td>
                  </tr>
