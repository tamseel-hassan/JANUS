...
export function AvailabilityReports() {
  ...
  const fetchData = async () => {
    ...
    const result = await safeFetch<{ devices: {id: number; name: string}[]; report_data: any }>(
      `/api/get_availability_reports.php?${query.toString()}`, {}, "AvailabilityReports"
    );
    if (result) {
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      <div className="flex items-center gap-4 border-b border-border-subtle pb-6">
        ...
        <div>
          ...
          <p className="text-text-muted">Generate and export historical SLA reports for network devices</p>
        </div>
      </div>
      ...
      <div className="bg-bg-raised rounded-2xl border border-border-subtle p-6">
        <form onSubmit={handleGenerate} className="grid grid-cols-1 md:grid-cols-4 lg:grid-cols-5 gap-4 items-end">
          <div className="lg:col-span-2">
            <label className="block text-sm font-medium text-text-muted mb-1.5">Select Device</label>
            <CustomSelect
              value={deviceId}
          ...
          <div>
            <label className="block text-sm font-medium text-text-muted mb-1.5">Time Range</label>
            <CustomSelect
              value={timeRange}
          ...
          <div>
            <label className="block text-sm mb-1.5 opacity-0 pointer-events-none" aria-hidden="true">&nbsp;</label>
            <Button
              type="submit"
              variant="primary"
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            <StatCard title="SLA Availability" value={`${data.availability}%`} />
            <StatCard title="Average RTT" value={data.avg_rtt ? `${data.avg_rtt} ms` : 'N/A'} />
            <StatCard title="Total Uptime" value={formatDuration(data.total_up_time)} color="text-emerald-400" />
            <StatCard title="Total Downtime" value={formatDuration(data.total_down_time)} color="text-red-400" />
          </div>

          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2 bg-bg-raised rounded-2xl border border-border-subtle p-6">
              <h2 className="text-lg font-bold text-foreground mb-6 flex items-center gap-2">
                <Activity className="w-5 h-5 text-accent-primary" /> Latency Over Time
              </h2>
              <div className="h-[400px] w-full">
                {data.rtt_data.length > 0 ? (
                  ...
                  <div className="w-full h-full flex items-center justify-center text-text-muted">No telemetry data available for this period.</div>
                )}
              </div>
            </div>
            ...
            <div className="bg-bg-raised rounded-2xl border border-border-subtle overflow-hidden flex flex-col">
              <h2 className="text-lg font-bold text-foreground p-6 pb-2 flex items-center justify-between">
                State Transitions
                <span className="text-xs bg-slate-700 text-text-muted px-2 py-1 rounded-2xl">{data.periods.length} events</span>
              </h2>
        ...
        <div className="bg-bg-raised rounded-2xl border border-border-subtle p-12 text-center">
          ...
          <p className="text-text-muted max-w-md mx-auto">Select a device and time range from the controls above to generate a comprehensive availability SLA report.</p>
        </div>
      )}
    </div>
  );
