...
import { Satellite, Network, Search, CheckCircle, Info, List, Terminal, AlertTriangle } from "lucide-react";
...
export default function VulnScanPage() {
  ...
  const handleScan = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/get_scan.php", {
        method: "POST",
    ...
    <div className="space-y-6">
      ...
      <div className="text-center mb-8">
        <h2 className="text-2xl font-bold text-foreground mb-2 flex justify-center items-center">
          <Satellite className="w-8 h-8 mr-3 text-accent-primary" /> Deep Packet Inspection Scanner
        </h2>
        <p className="text-text-muted">Janus Scanning Engine | Service Version Detection | OS Fingerprinting</p>
      </div>

      <div className="max-w-3xl mx-auto bg-bg-raised border border-border-subtle rounded-2xl overflow-hidden p-6">
        <form onSubmit={handleScan} className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-text-muted mb-2">Target Host / IP Address</label>
            <div className="relative flex">
              <span className="inline-flex items-center px-4 rounded-l-md border border-r-0 border-border-subtle bg-bg-main text-accent-primary">
              ...
              <button 
                type="submit" 
                disabled={scanning}
                className="inline-flex items-center px-6 rounded-r-md bg-accent-primary hover:bg-accent-hover disabled:opacity-50 text-foreground font-bold transition-colors"
              >
                <Search className="w-5 h-5 mr-2" /> INITIATE SCAN
              </button>
            </div>
          </div>
          <div className="text-sm text-text-muted flex items-center">
            <Info className="w-4 h-4 mr-1" /> This scan performs active service fingerprinting (-sV). Please allow 15-45 seconds.
          </div>
        </form>
      </div>
      ...
      {scanning && (
        <div className="flex flex-col items-center justify-center p-12">
          ...
          <h4 className="text-xl font-bold text-foreground mb-2">Analyzing Network Topology...</h4>
          <p className="text-text-muted">Performing TCP handshake and banner grabbing</p>
        </div>
      )}
        ...
        <div className="space-y-6 animate-pulse-once">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-bg-raised border-t-4 border-t-green-500 border-border-subtle rounded-2xl p-6 text-center">
              <h6 className="text-text-muted text-xs font-bold uppercase tracking-wider mb-3">Target Status</h6>
            ...
            <div className="bg-bg-raised border-t-4 border-t-yellow-500 border-border-subtle rounded-2xl p-6 text-center">
              <h6 className="text-text-muted text-xs font-bold uppercase tracking-wider mb-3">Open Ports</h6>
              <h3 className="text-2xl font-bold text-foreground mb-1">{result.ports.length}</h3>
            ...
            <div className="bg-bg-raised border-t-4 border-t-blue-500 border-border-subtle rounded-2xl p-6 text-center">
              <h6 className="text-text-muted text-xs font-bold uppercase tracking-wider mb-3">Scan Type</h6>
          ...
          <div className="bg-bg-raised border border-border-subtle rounded-2xl overflow-hidden">
            <div className="p-4 border-b border-border-subtle bg-bg-main flex justify-between items-center">
              <h5 className="font-semibold text-foreground flex items-center mb-0">
                <List className="w-5 h-5 mr-2 text-accent-primary" /> Detected Services
              </h5>
              <span className="bg-bg-card border border-border-subtle px-3 py-1 rounded-2xl text-xs font-mono text-foreground">{result.address}</span>
            </div>
              ...
              <table className="w-full text-left text-sm">
                ...
                <tbody className="divide-y divide-border-subtle">
                  {result.ports.map((port: any, idx: number) => (
                    <tr key={idx} className="hover:bg-bg-card/50 transition-colors">
                      <td className="px-6 py-4 font-bold text-blue-400">{port.port}</td>
                      <td className="px-6 py-4 text-foreground">{port.protocol}</td>
                      <td className="px-6 py-4">
                        <span className="bg-green-900/50 text-green-400 px-2 py-1 rounded-2xl text-xs font-bold">{port.state}</span>
                      </td>
                      <td className="px-6 py-4 font-bold text-foreground">{port.service}</td>
                      <td className="px-6 py-4">
                        {port.product && <span className="text-foreground block">{port.product}</span>}
                        <span className="text-text-muted text-xs font-mono">({port.version})</span>
                      </td>
                      <td className="px-6 py-4">
                        <span className={`inline-block px-2 py-1 rounded-2xl text-xs font-bold uppercase mb-1 ${
                          port.risk_level === 'critical' ? 'bg-red-900/50 text-red-400' :
                          port.risk_level === 'high' ? 'bg-orange-900/50 text-orange-400' :
                          port.risk_level === 'medium' ? 'bg-yellow-900/50 text-yellow-400' :
                          'bg-green-900/50 text-green-400'
                        }`}>
                          {port.risk_level}
                        </span>
                        <small className="block text-text-muted text-xs">{port.desc}</small>
                      </td>
                    </tr>
                  ))}
                  {result.ports.length === 0 && (
                    <tr><td colSpan={6} className="px-6 py-8 text-center text-text-muted">No open ports detected.</td></tr>
                  )}
                </tbody>
              </table>
            </div>
