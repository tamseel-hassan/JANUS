...
import { Shield, Plus, ShieldAlert, Activity, CheckCircle2, AlertCircle, Loader2, Server } from 'lucide-react';
          ...
          <form onSubmit={handleBlockIP} className="space-y-4 relative z-10">
            ...
            <div>
              ...
              <select
                ...
                onChange={e => setBlockForm({ ...blockForm, firewall_ip: e.target.value })}
              ...
              >
                ...
                {firewalls.filter(f => f.is_active).map(f => (
                  <option key={f.firewall_ip} value={f.firewall_ip}>
                    {f.firewall_ip} (added by {f.username})
                  </option>
                ))}
              </select>
            </div>
            ...
            <button
              ...
              className="w-full bg-red-600 hover:bg-red-700 text-foreground rounded-2xl px-4 py-3 font-semibold transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex justify-center items-center gap-2 mt-4"
            >
              <ShieldAlert className="w-5 h-5" /> Block IP on Firewall
            </button>
        ...
        <div className="bg-slate-800 rounded-2xl border border-slate-700 p-6 relative overflow-hidden">
          ...
          <form onSubmit={handleAddFirewall} className="space-y-4">
            ...
            <button
              ...
              className="w-full bg-slate-700 hover:bg-slate-600 border border-slate-600 hover:border-emerald-500 text-foreground rounded-2xl px-4 py-3 font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex justify-center items-center gap-2 mt-4"
            >
