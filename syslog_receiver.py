...
DB_USER = os.getenv('DB_USER', 'janus_user')
DB_PASS = os.getenv('DB_PASS', 'Janus@DB@2026')
DB_NAME = os.getenv('DB_NAME', 'alogin')
LOG_DIR = '/var/log/janus_siem'
SYSLOG_HOST = '0.0.0.0'
SYSLOG_PORTS = [5140]  # Listen on 5140 (host mode); 514 is taken by rsyslog
...
class SyslogReceiver:
    ...
    def connect_db(self):
        for attempt in range(1, 6):
            try:
                ...
                logging.error(f"Database connection attempt {attempt}/5 failed: {e}")
                if attempt < 5:
                    time.sleep(3 * attempt)
        return False

    def ensure_db(self):
        """Ping the connection and reconnect if needed (fixes broken pipe after idle)."""
        try:
    ...
    def write_to_db(self, source_id, appliance_type, source_ip, message, facility=1, severity=6):
        try:
            if not self.ensure_db():
                return False
            ...
            cursor.execute(
                ...
                INSERT INTO syslog_entries (
                    source_id, appliance_type, source_ip, facility, severity, message,
    ...
    def parse_syslog(self, data):
        ...
        try:
            ...
            if match:
                ...
                facility = pri // 8
                severity = pri % 8
                return match.group(2).strip(), facility, severity
            return message, facility, severity
        except Exception:
            return data.decode('utf-8', errors='ignore'), facility, severity

    def process_message(self, data, source_ip):
        try:
            message, facility, severity = self.parse_syslog(data)
            if not message:
                return

            source = self.check_source(source_ip)
            if source:
                self.write_to_db(source['id'], source['appliance_type'], source_ip, message, facility, severity)
                self.write_to_file(source_ip, source['appliance_type'], message)
                logging.info(f"✔ {source_ip} ({source['appliance_type']}) [Sev:{severity}]: {message[:50]}")
            else:
