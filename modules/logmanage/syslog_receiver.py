#!/usr/bin/env python3
import socket, sys, re, pymysql, os, logging
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('/var/log/janus_siem/receiver.log'),
...
class SyslogReceiver:
    ...
    def write_to_db(self, source_id, appliance_type, source_ip, message):
        try:
            if not self.db or not self.db.open:
                self.connect_db()
            cursor = self.db.cursor()
            cursor.execute("INSERT INTO syslog_entries (source_id, appliance_type, source_ip, message) VALUES (%s, %s, %s, %s)",
    ...
    def start(self):
        logging.info("=== Janus SIEM Starting ===")
